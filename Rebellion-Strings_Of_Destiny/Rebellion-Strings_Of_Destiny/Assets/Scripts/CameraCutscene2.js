...
function Update () {	
	...
	switch(evento){
		case 0:
			...
			lilianScript.acao = 2;
			if(blinker.renderer.material.color.a <= 0 && timer >= 6){
				timer = 0;
				timer2 = 0;
				evento++;
			}
...
function OnGUI(){
	switch(evento){
		...
		case 2:
			...
			posAux.y = Screen.height - posAux.y; // Inverter o Y (Por causa do OpenGL);
			GUI.skin = skin2;
			GUI.Box(Rect(posAux.x - (130 / 2), posAux.y - 100, 130, 50), "\nLilian, SOCORRO!!!"); // Posicionar sobre o player (Na cabeça dele);
			if(timer >= 3){
				timer = 0;
				evento++;
			}
		...
		case 3:
			...
			break;
		case 4:
			posAux = camera.WorldToScreenPoint(arcanjo.transform.position); // Converter coordenadas do mundo em coordenadas de tela;
			posAux.y = Screen.height - posAux.y; // Inverter o Y (Por causa do OpenGL);
			GUI.skin = skin3;
			GUI.Box(Rect(posAux.x - (130 / 2), posAux.y - 100, 130, 50), "Calada humana, \ndesapareça daqui!!!!"); // Posicionar sobre o player (Na cabeça dele);
			if(timer >= 4){
				timer = 0;
				evento++;
			}
		...
		case 5:
			lilianScript.acao = 1;
			if(timer < 4)
				arcanjoScript.estado = 4;
			if(timer >= 3 && timer < 4)
				arcanjoScript.estado = 0;
			if(timer >= 5){
				timer = 0;
				timer2 = 0;
				evento++;
		...
		case 6:
			...
			if(blinker.renderer.material.color.a <= 0 && timer >= 3){
